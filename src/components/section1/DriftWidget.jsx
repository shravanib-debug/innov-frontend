...
const DriftWidget = ({ data, loading }) => {
    const currentDrift = data?.score ?? 0;
    const status = data?.status ?? 'normal';
    const driftColor = status === 'critical' ? '#ef4444' : status === 'warning' ? '#eab308' : '#22c55e';

    // Build trend from API data if available, otherwise show empty
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={trendData}>
                            ...
                            <Tooltip
                                ...
                                formatter={(val) => [Number(val).toFixed(3), 'Drift Score']}
                            />
                            ...
                            <Line type="monotone" dataKey="value" stroke={driftColor} strokeWidth={2} dot={{ r: 3, fill: driftColor }} />
                        </LineChart>
                    </ResponsiveContainer>
                </div>
            ) : (
                <div className="h-28 mb-4 flex flex-col items-center justify-center">
                    <div className="w-16 h-16 rounded-full border-4 border-[#2a201a] flex items-center justify-center mb-2">
                        <span className="text-xl font-bold" style={{ color: driftColor }}>{currentDrift.toFixed(2)}</span>
                    </div>
                    <p className="text-[10px] text-[#5a4a3a]">Current drift score</p>
                </div>
            )}
